...
void MainWindow::appendToSocketList(QTcpSocket* socket)
{
    ...
    displayMessage(QString("INFO :: %1번 사용자가 방에 들어왔습니다.").arg(socket->socketDescriptor()));
}
...
void MainWindow::readSocket()
{
    ...
    if(fileType=="attachment"){ // 메시지 유형이 "첨부파일"인 경우
        ...
        if (QMessageBox::Yes == QMessageBox::question(this, "QTCPServer", QString("사용자가 보낸 파일을 받으시겠습니까?").arg(socket->socketDescriptor()).arg(size).arg(fileName))) // 파일을 받는 경우
        {
...
void MainWindow::discardSocket() // 소켓 삭제
{
    ...
    if (it != connection_set.end()){
        displayMessage(QString("사용자가 나갔습니다.").arg(socket->socketDescriptor()));
